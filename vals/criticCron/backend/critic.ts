...
export async function critiquePost(postUrl: string): Promise<{
  ...
  // Omit the summary's body copy: the untruncated content is appended below, and
  // shipping both let the model critique the truncated one.
  const postSummary = formatPost(post, post.content ? 0 : 1000);
...
// Send both parts. HTML-only leaves the text/plain alternative as Val Town's
// "Email sent from Val Town" placeholder, so text clients and anything reading
// the mailbox over JMAP sees nothing of the critique — and the markdown is
...
export async function emailCritique(
  ...
  const html = buildEmailHtml(title, url, critique.html, prUrl);
  const prLine = prUrl ? `🔗 Draft PR with suggested links: ${prUrl}\n\n` : "";
  await email({
    subject: `Critique: ${title}`,
    html,
    text: `${title}\n${url}\nCritique from your writing mentor\n\n${prLine}${critique.markdown}\n\n— Generated by criticCron`,
  });
}
...
export async function processFeed(): Promise<{ processed: string[]; skipped: number }> {
  ...
  for (const entry of eligible) {
    try {
      ...
      await emailCritique(result.title, result.url, result.critique, prUrl);
